...
if __name__ == "__main__":
    ...
    model.fc = nn.Linear(num_ftrs, 2)  # 全连接层改为不同的输出
    device = "cuda" if torch.cuda.is_available() else "cpu"

    # 2. 加载模型参数

    model_loc = "BEST_resnet_epoch_6_acc_71.4.pth"
    ...
    # 将输出保存到exel中，方便后续分析
    label_names = ["0", "1"]     # 可以把标签写在这里
    df_pred = pd.DataFrame(data=pred, columns=label_names)


    df_pred.to_csv('pred_result_1.csv', encoding='gbk', index=False)
    print("Done!")
